...
export function SimulationModal({ open, onClose }: SimulationModalProps) {
  ...
  const runSimulation = () => {
    ...
    setTimeout(() => {
      ...
      const panelCount = Math.floor(Number.parseFloat(roofArea) / 4) || 30
      const installationCost = panelCount * 80000
      const roiYears = Math.ceil(installationCost / (Number.parseFloat(cost) || 20000))
      ...
      const generateMonthlyData = () => {
        ...
        return Array.from({ length: 12 }, (_, i) => {
          ...
          return Math.floor(baseValue * seasonFactor * (0.8 + Math.random() * 0.4))
        })
      }
      ...
      setSimulationData({
        ...
        yearlyComparison: {
          consumption: Array.from({ length: 12 }, () => Math.floor(15 + Math.random() * 30)),
          production: Array.from({ length: 12 }, () => Math.floor(10 + Math.random() * 30)),
        },
    ...
    <Modal open={open} onClose={onClose} className="w-full">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="p-6 border-b flex justify-between items-center">
          <h2 className="text-2xl font-bold text-primary">Simulation Solaire</h2>
          <button
            onClick={onClose}
        ...
        {step === 1 && (
          <div className="flex flex-col md:flex-row">
            ...
            <div className="md:w-1/3 p-6 border-r">
              <h3 className="text-lg font-bold text-primary mb-4">Informations de base</h3>
              <div className="space-y-4">
                ...
                {selectedLocation && (
                  <div className="text-xs text-green-600 mt-2">
                    ✓ Emplacement sélectionné: {selectedLocation.lat.toFixed(4)}, {selectedLocation.lng.toFixed(4)}
                  </div>
                )}
              </div>
            </div>
              ...
              <div className="p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                  <div className="border rounded-lg p-4">
                    ...
                    <div className="text-3xl font-bold text-accent">{simulationData?.panels}</div>
                  ...
                  <div className="border rounded-lg p-4">
                    ...
                    <div className="text-3xl font-bold text-accent">{simulationData?.cost}</div>
                  ...
                  <div className="border rounded-lg p-4">
                    ...
                    <div className="text-3xl font-bold text-accent">{simulationData?.roi}</div>
                ...
                <div className="flex justify-between">
                  <Button onClick={() => setStep(1)} variant="outline" className="border-primary text-primary">
                    <ChevronLeft className="mr-2 h-4 w-4" /> Modifier l'emplacement
                  </Button>
                  <div className="space-x-3">
                    <Button variant="outline" className="border-primary text-primary">
                      Imprimer
                    </Button>
                    <Button className="bg-primary hover:bg-secondary text-white">Réservez Un Appel</Button>
                  </div>
                </div>
              </div>
            )}
